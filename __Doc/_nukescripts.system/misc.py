...
def toggle_monitor_output():
  ...
  if viewer is not None:
    enableKnob = viewer.node().knob('MonitorOutEnable')
...
def print_callback_info(verbose=False, callbackTypes=None):
  ...
  maxTargetNameLen = max( map( len,' '.join( [ (k+'').join( callback_defs[k].keys() ) for k in callback_defs.keys() ] ).split(' ') ) )

  indent = (maxTargetNameLen+8)*' '

  sortedCallbackTypes = sorted( callback_defs.keys() )
  for callbackType in sortedCallbackTypes:
    for callbackTarget in callback_defs[callbackType].keys():
