...
def pluginAddPath(args, addToSysPath = True):
  ...
  if type(args) != types.TupleType and type(args) != types.ListType:
    args = [args]
...
def dependencies(nodes, what = _nuke.INPUTS | _nuke.HIDDEN_INPUTS | _nuke.EXPRESSIONS):
  ...
  if type(nodes) != types.TupleType and type(nodes) != types.ListType:
    nodes = [nodes]
...
def dependentNodes(what = _nuke.INPUTS | _nuke.HIDDEN_INPUTS | _nuke.EXPRESSIONS, nodes = [], evaluateAll = True):
  """ List all nodes referred to by the nodes argument. 'what' is an optional integer (see below).
