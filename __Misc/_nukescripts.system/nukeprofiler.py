...
class NukeProfiler:
  ...
  def OpenTag(self, tagName, optionsDict = {}, closeTag = False):
    ...
    if closeTag:
      xmlString += "\n"
    self._indent += 1
    for dictKey, dictValue in optionsDict.iteritems():
  ...
  def WriteDictInner(self, dictToWrite):
    xmlString = ""
    for dictKey, dictValue in dictToWrite.iteritems():
      xmlString += self.indentString() + "<" + dictKey + ">" + str(dictValue) + "</" + dictKey + ">\n"
    return xmlString
  ...
  def NodeProfile(self, nukeNode, maxEngineVal):
    ...
    for catName, catVal in profileCategories.iteritems():
  ...
  def addFrameProfileAndResetTimers(self):
    ...
    for key, value in maxInfo.iteritems():
