...
def createEverything(cameraTracker):
  ...
  dummyCamera = nuke.createNode('Camera', '', False)
  # creating our 4 dots
...
def _createLensDistortionNode(cameraTracker, invertDistortion):
  ...
  lensDistort = nuke.createNode('LensDistortion', '', False)
  lensDistort.setInput(0, cameraTracker.input(0))
  link = cameraTracker["linkOutput"].getValue()
  _copyKnob(cameraTracker, "lensType",              lensDistort, "lensType",              link)
  _copyKnob(cameraTracker, "distortion1",           lensDistort, "distortion1",           link)
  _copyKnob(cameraTracker, "distortion2",           lensDistort, "distortion2",           link)
  _copyKnob(cameraTracker, "distortionCenter",      lensDistort, "distortionCenter",      link)
  _copyKnob(cameraTracker, "anamorphicSqueeze",     lensDistort, "anamorphicSqueeze",     link)
  _copyKnob(cameraTracker, "asymmetricDistortion",  lensDistort, "asymmetricDistortion",  link)
  lensDistort['invertDistortion'].setValue(invertDistortion)
  _copyKnob(cameraTracker, "filter",                lensDistort, "filter",                False)
  _copyKnob(cameraTracker, "cardScale",             lensDistort, "cardScale",             False)
  _copyKnob(cameraTracker, "a",                     lensDistort, "a",                     False)
  _copyKnob(cameraTracker, "b",                     lensDistort, "b",                     False)
  _copyKnob(cameraTracker, "c",                     lensDistort, "c",                     False)
...
def _copyLinkableXYAnimCurve(linkableSrc, linkableDst):
  ...
  for i in xrange(0, linkKnobSrc.getNumKeys(int(linkableSrcIndices[0]))):
...
def exportTracker(cameraTracker):
  ...
  for linkableDst, userTrackEnabled in trackEnabledDict.iteritems():
