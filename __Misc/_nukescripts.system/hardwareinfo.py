...
class HardwareInfo:
  ...
  def SafeCall(self, f, *positional, **keyword):
    try:
      return apply(f, positional, keyword)
    except Exception, e:
  ...
  def parseProcInfo(self, info):
    ...
    if not success:
      return ([{}], False)
    import commands
    cpuinfo = commands.getoutput("cat " + info)
  ...
  def ParseProcVersion(self):
    if self.testCatCommand("/etc/redhat-release"):
      import commands
      osVersion = commands.getoutput("cat /etc/redhat-release")
      self._standardDict[gOSVersion] = osVersion
    elif self.testCatCommand("/etc/lsb-release"):
      import commands
      lsbInfo = commands.getoutput("cat /etc/lsb-release")
      ...
      import commands
      osVersion = commands.getoutput("cat /proc/version")
  ...
  def parseProcSimple(self, file, entry):
    ...
    if os.path.exists(file):
      self._standardDict[entry] = commands.getoutput("cat " + file)
    else:
  ...
  def getRegistryNumSubKeys(self, key, subkey):
    try:
      ...
      return _winreg.QueryInfoKey(handle)[0]
    except:
  ...
  def getRegistryValue(self, key, subkey, value):
    try:
      ...
      key = getattr(_winreg, key)
      handle = _winreg.OpenKey(key, subkey)
      (result, type) = _winreg.QueryValueEx(handle, value)
      return result
    except:
      self.gLogStr += "Failed to find " + str(key) + ", " + str(subkey) + ", " + str(value) + "\n"
      return "Unknown"
  ...
  def printDict(self, pDict, handle, indentLevel = 1):
    ...
    for key in sorted(pDict):
      ...
      if type(elem) == dict: #nest
        ...
        #  elem = elem.replace(u"\u001B", "")
        try:
          for badch in u"<>&": #make sure anything with these is escaped, so XML doesn't screw up
            if badch in unicode(elem) and not unicode(elem).startswith("<![CDATA"):
              elem = "<![CDATA[\n%s\n]]>" % elem
            for c in str(elem):
              if ord(c) > 128:
