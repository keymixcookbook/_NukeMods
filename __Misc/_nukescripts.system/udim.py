...
import nuke, os, re, sys, math, time
...
class UDIMErrorDialog(QDialog):
  def __init__(self, parent=None, error_msg="", udim_label="UDIM"):
    ...
    hlayout = QHBoxLayout()
    hlayout.insertSpacerItem(0, QSpacerItem(1, 1, QSizePolicy.Expanding, QSizePolicy.Minimum))
...
class UDIMOptionsDialog(QDialog):
  ...
  def __init__(self, parent=None, parsing_func=parseUdimFile, udim_label="UDIM"):
    ...
    # Create widgets
    self.UdimList = QTableWidget(0, 3, self)
    self.AddFilesButton = QPushButton("Add Files")
    self.ConflictLabel = QLabel("")
    self.Separator = QFrame()
    self.ReadModeComboBox = QComboBox()
    self.PostageStampCheckBox = QCheckBox("postage stamp")
    self.GroupNodesCheckBox = QCheckBox("group nodes")
    ...
    self.AddFilesButton.setSizePolicy(QSizePolicy.Fixed, QSizePolicy.Fixed)
    self.OkButton.setSizePolicy(QSizePolicy.Fixed, QSizePolicy.Fixed)
    self.CancelButton.setSizePolicy(QSizePolicy.Fixed, QSizePolicy.Fixed)
    self.ReadModeComboBox.setSizePolicy(QSizePolicy.Fixed, QSizePolicy.Fixed)
    ...
    self.UdimList.setEditTriggers(QAbstractItemView.NoEditTriggers)
  ...
  def updateTableWidget(self):
    ...
    # sort the udim list
    self.UdimMap.sort(compare_UDIMFile)
    ...
    if self.UdimConflict :
      ...
      self.ConflictLabel.setPalette(QPalette())
    ...
    for u in self.UdimMap:
      ...
      if u.uv == None:
        udim_id = QTableWidgetItem( str(u.udim) )
      else:
        u_coord,v_coord = u.uv
        udim_id = QTableWidgetItem( "(%(u)02d,%(v)02d)" % {"u":u_coord, "v":v_coord} )

      udim_filename = QTableWidgetItem( u.filename )

      if u.enabled and u.udim in udimCountMap and udimCountMap[u.udim] > 1:
        udim_id.setForeground(QBrush(Qt.yellow))
        udim_filename.setForeground(QBrush(Qt.yellow))
      ...
      if u.enabled == True:
        ...
        udim_id.setForeground(QBrush(Qt.darkGray))
        udim_filename.setForeground(QBrush(Qt.darkGray))
  ...
  def cellChanged(self, row, column):
    ...
    if column == 2:
      ...
      if checked.checkState() == Qt.Checked:
